 ...
 *
 * The result workbook is copied from the original and only the MD5 cells
 * are updated, preserving the original workbook layout and formatting.
 */
...
%macro _process_zip_member(row_id=);
    ...
    proc sql noprint;
        ...
        from work._tmp_results where row_id=&row_id;
    quit;
    ...
    data work._tmp_zip_result_&row_id;
        ...
        length member $2048 member_file $1024 first_member $2048
               member_md5 $32 first_md5 $32 mem_ref $8 extract_ref $8;

        status='OK'; message=''; match_count=0; first_md5=''; first_member='';
        did=dopen('inzip');
                ...
                if upcase(member_file)=upcase(transfer_name) then do;
                    ...
                    if rc2 ne 0 then do;
                        ...
                        if missing(member_md5) then do;
                            ...
                            first_md5=member_md5; first_member=member;
                        end;
                        else if member_md5 ne first_md5 then do;
                            status='ERROR'; message='Duplicate ZIP members have different MD5 values.';
                        end;
                    end;
        ...
        if status='OK' then do;
            ...
            rc1=filename(extract_ref); rc2=filename('xout');
        end;
...
%macro prepare_transfer(
    ...
    %local _dircol _filecol _md5col _errors _zip_rows _zip_n _z _zip_row
           _xl_dircol _xl_filecol _xl_md5col;
    ...
    proc datasets library=work nolist; delete md5_result; quit;

    proc import datafile="&xlsx" out=work._tmp_raw dbms=xlsx replace;
        sheet="&sheet"; getnames=yes;
    run;

    %_resolve__excel_columns(data=work._tmp_raw,
        directory_col=&directory_col,file_col=&file_col,md5_col=&md5_col);
    ...
    data work._tmp_input; set work._tmp_raw; row_id=_n_; run;

    data work._tmp_results;
        set work._tmp_input;
        length directory_path $1024 file_name $1024 source_type $3 md5 $32
               transfer_path $2048 transfer_name $1024 status $8 message $500 fileref $8;
        ...
        if missing(directory_path) then do; status='ERROR'; message='DIRECTORY_PATH is required.'; end;
        else if missing(file_name) then do; status='ERROR'; message='FILE_NAME is required.'; end;

        if status='OK' and source_type='ZIP' and not whole_zip then status='ZIP';
            ...
            fileref='srcfile'; rc=filename(fileref,transfer_path);
            if rc ne 0 then do; status='ERROR'; message=cats('Cannot assign source file: ',sysmsg()); end;
            else if not fexist(fileref) then do; status='ERROR'; message='Source file does not exist.'; end;
            else do;
                md5=hashing_file('MD5',fileref,4);
                if missing(md5) then do; status='ERROR'; message=cats('MD5 calculation failed: ',sysmsg()); end;
            end;
            rc=filename(fileref);
        end;
        keep row_id directory_path file_name md5 source_type transfer_path transfer_name status message;
    run;

    proc sql noprint;
        select row_id into :_zip_rows separated by ' ' from work._tmp_results where status='ZIP';
    quit;
    ...
    data _null_;
        set work._tmp_results end=eof; retain errors 0;
        if status='ERROR' then do;
            errors+1;
            putlog 'ERROR: Manifest preparation failed. ' row_id= directory_path= file_name= message=;
        end;
    ...
    data &out; set work._tmp_results; drop status message; run;

    /* Copy the original workbook byte-for-byte first. */
    filename _xlsrc "&xlsx" recfm=n lrecl=1048576;
    filename _xldst "&result_xlsx" recfm=n lrecl=1048576;
    data _null_;
        rc=fcopy('_xlsrc','_xldst');
        if rc ne 0 then do;
            putlog 'ERROR: Could not copy original workbook: ' sysmsg();
            call symputx('_errors',1,'L');
        end;
    run;
    filename _xlsrc clear;
    filename _xldst clear;
    %if &_errors>0 %then %goto cleanup;

    /*
     * Update only the MD5 cells in the copied workbook.
     * SCAN_TEXT=NO enables Excel UPDATE and avoids rebuilding the sheet,
     * so original headers, column order and formatting remain untouched.
     */
    libname _tmpxl excel "&result_xlsx" scan_text=no;

    proc contents data=_tmpxl."&sheet.$"n
        out=work._tmp_xlcols(keep=name varnum) noprint;
    run;

    proc sql noprint;
        select nliteral(name) into :_xl_dircol trimmed
          from work._tmp_xlcols where varnum=&directory_col;
        select nliteral(name) into :_xl_filecol trimmed
          from work._tmp_xlcols where varnum=&file_col;
        select nliteral(name) into :_xl_md5col trimmed
          from work._tmp_xlcols where varnum=&md5_col;
    quit;

    data _null_;
        set work._tmp_results;
        length sql $7000 dirq fileq $2200;
        dirq=tranwrd(strip(directory_path),"'","''");
        fileq=tranwrd(strip(file_name),"'","''");
        sql=cats('proc sql; update _tmpxl."&sheet.$"n set ',
                 "&_xl_md5col",'=',quote(strip(md5),"'"),
                 ' where ',"&_xl_dircol",'=',quote(strip(dirq),"'"),
                 ' and ',"&_xl_filecol",'=',quote(strip(fileq),"'"),'; quit;');
        call execute(sql);
    run;
